// ========== lib/screens/chat_screen_mobile.dart ==========
import 'dart:convert';
import 'dart:io';
...
class _ChatScreenMobileState extends ConsumerState<ChatScreenMobile>
  ...
  void _toggleMediaPanel() { if (_showMediaPanel) { _mediaController.reverse(); } else { _mediaController.forward(); } setState(() => _showMediaPanel = !_showMediaPanel); }
  void _sendVideoMessage() async {
    ...
    if (video == null) return; final bytes = await video.readAsBytes(); final name = video.name;
    final now = TimeOfDay.now(); final time = '${now.hour}:${now.minute.toString().padLeft(2, '0')}';
    ...
    if (!mounted) return;
    setState(() => _messages.add({'type': 'video_msg', 'fileName': name, 'text': '🎬 Видеосообщение', 'isMe': true, 'time': time})); _scrollDown();
  }
  ...
  void _showStickerPicker() {
    showModalBottomSheet(context: context, backgroundColor: Theme.of(context).colorScheme.surface,
      ...
      builder: (ctx) => Padding(padding: const EdgeInsets.all(16), child: Column(mainAxisSize: MainAxisSize.min, children: [
        Text('Стикеры', style: TextStyle(color: Theme.of(context).colorScheme.onSurface, fontWeight: FontWeight.w600)), const SizedBox(height: 12),
  ...
  void _sendVideo() async {
    ...
    await _fileService.saveFile(chatId: widget.contactId, fileName: name, bytes: Uint8List.fromList(bytes), isMe: true, time: time, key: _sharedKey);
    if (!mounted) return;
    setState(() => _messages.add({'type': 'video', 'fileName': name, 'text': '🎬 Видео', 'isMe': true, 'time': time})); _scrollDown();
  }
  ...
  void _sendPhoto() async {
    ...
    if (photo == null) return; final bytes = await photo.readAsBytes(); final now = TimeOfDay.now(); final time = '${now.hour}:${now.minute.toString().padLeft(2, '0')}';
